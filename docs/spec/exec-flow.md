...
选项可附带条件（`if` 属性）——UI 层根据当前 `state_vars` 评估条件，决定哪些选项可交互、哪些置灰。全部选项条件不满足时，应全部以可选样式展示（兜底防卡死）。

禁用选项被选中时，UI 应提示用户重新选择，不向引擎发送。

> **引擎数据支持**：`options` 事件中每个选项携带 `conditions`（`branch → 条件表达式` 映射）和 `labels`（选项文本）。引擎通过 `state` 事件提供当前 `state_vars`。UI 层可自行评估条件、渲染置灰样式、计算显示值。条件评估使用与引擎一致的运算符（`==` `!=` `>=` `<=` `>` `<`）和 `choice_dict > state_vars` 优先级。
