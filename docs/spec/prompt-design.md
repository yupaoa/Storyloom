...
**段数与 bridge：**
- 本轮生成 {MIN}-{MAX} 个叙事段。bridge 放在总段数的约 {RATIO} 处——总段数 × {RATIO}，向下取整。例如：总 40 段 → bridge 在第 30 段后。
...
|------|------|------|
| 2026-07-04 | 初始版本 | — |
| 2026-07-04 | 格式示例重构：bridge 从 54% 移至 75%、checkpoint 补全 route 条件、禁止列表重排序、增加编号重启规则 | 5 次测试发现 LLM 系统性地将 bridge 放在 44-47%、缺失 route 分支、编号接续示例。根因：示例与数值规则矛盾（示例 54% vs 规则 75%），LLM 倾向模仿示例。 |
| 2026-07-04 | 增加 state 条件语法规则、分支编号连续性规则、对话代词禁止规则；bridge 公式改为比例优先 | 第二轮 5 次测试结构问题清零，但新暴露：1) T05 用 `choice == "A"` 代替 `confrontation == 1`；2) T01/T02/T05 分支块编号重复（都从同一数字开始）；3) T01 对话段用 `你:` 代替 `林焰:`。bridge 公式改为"总段数×0.75 向下取整"消除绝对数字锚定。 |
